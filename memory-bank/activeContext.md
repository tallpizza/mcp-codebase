...
- LSP 클라이언트 구현 (src/services/lspClient.ts)
  - TypeScript 언어 서버와 통신
  - 코드 심볼 추출 기능 구현
  - 문서 심볼 처리 구현
  - TSServer 경로 옵션 오류 수정
...
- 코드 청킹 API 오류 해결
  - LSP 클라이언트 초기화 문제 해결 (tsserver 경로 옵션 제거)
  - 오류 처리 및 로깅 개선
...
3. **코드 청킹 서비스 개선**
   - 라인 번호 계산 버그 수정 (1-based indexing)
   - 상대 경로 변환 로직 추가
   - 청크 추출 정확도 향상
...
1. **임베딩 생성 안정성**
   ...
   - 문제: OpenAI API 입력 유효성 검사 오류
   - 해결: 전처리 단계에서 엄격한 유효성 검사 추가
   - 결과: 임베딩 생성 실패율 감소

2. **파일 경로 처리**

   - 문제: 절대 경로 사용으로 인한 이식성 문제
   - 해결: 상대 경로 사용 및 임베딩에 경로 정보 포함
   - 결과: 코드 문맥 이해도 향상 및 이식성 개선

3. **성능 최적화**
   - 문제: 대량의 코드 청크 처리 시 성능 저하
   - 해결: 배치 처리 및 병렬 처리 최적화
   - 진행 중: 메모리 사용량 최적화
